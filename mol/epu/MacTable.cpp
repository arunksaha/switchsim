...
MacTable::showMacEntries() const
{
	...
	for( MacEntrySetConstItr it = macEntries_.begin(); it != macEntries_.end(); it++ )
	{
...
void
MacTable::learnSa( MacAddr const & mac, FwdDomainId const & fdId, EpuId const & epuId )
{
    MacEntryKey mek;
    mek.mac_        = mac;
    mek.domainId_   = fdId;
    ...
    else
    {
        ...
        if( meExisting->epuId_ != epuId )
        {
            ...
            cout << "\t\t\t MAC Move: Mac Addr = " << mac << 
                ", " << VbNode.tokenString() << " Id = " << fdId <<
                "; Old " << EpuNode.tokenString() << " = " << meExisting->epuId_ <<
                ", New " << EpuNode.tokenString() << " = " << epuId <<
                endl;
...
MacEntry *
MacTable::lookupDa( MacAddr const & mac, FwdDomainId const & fdId ) const
{
    MacEntryKey mek;
    mek.mac_        = mac;
    mek.domainId_   = fdId;
...
MacEntry *
MacTable::createMacEntry( MacEntryKey const & mek )
{
...
void
MacTable::deleteMacEntry( MacEntryKey const & mek )
{
...
MacEntry *
MacTable::findMacEntry( MacEntryKey const & mek ) const
{
...
void
MacTable::dumpMacEntry( MacEntry const * me ) const
{
    cout << (me->key_).mac_ << " \t\t\t ";
...
bool
MacEntryKey::operator==( MacEntryKey const & rhs ) const
{
...
bool
MacEntry::operator==( MacEntry const & rhs ) const
{
