...
void
EpuMgr::showEpus( EpuId const & epuId, bool const & counterFlag ) const
{
	EpuSetConstItr 	it;
	EpuId			eid;
	Epu			  	*e;
...
void
EpuMgr::dumpHeader( bool const & counterFlag ) const
{
	cout << "------------------------------------------------------------------------------" << endl;
    cout << EpuNode.tokenString() << " Id \t\t " <<
        PortNode.tokenString() << " Id \t " <<
...
void
EpuMgr::dumpEpu( Epu const * e, bool const & counterFlag ) const
{
    assert( e );
...
Epu *
EpuMgr::createEpu( EpuId const & epuId )
{
	Epu * e = new Epu;
	assert( e );
	e->epuId( epuId ); 
...
void
EpuMgr::deleteEpu( EpuId const & epuId )
{
	Epu * e = findEpu( epuId );
...
Epu *
EpuMgr::findEpu( EpuId const & epuId ) 
{
