...
void
SimulParserHandler::rxFrame( PortId pId, Frame f )
{
    if( pId >= portMgr()->numPorts() )
    {
        cerr << "Port " << pId << "does not exist, please specify a port number in 0-" << portMgr()->numPorts() << endl;
        return;
    }
...
ParserMain *
SimulParserHandler::parserMain() const
{
