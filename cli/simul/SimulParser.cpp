...
namespace SimulParserNameSpace
{
	...
	void acceptSimulHelpEol( const char * a, const char * b )
	{
		HelpFlag = true;
	}
	...
	void doSimulExitHelp( const char * a, const char * b )
	{
	...
	void doSimulEndHelp( const char * a, const char * b )
	{
		printHelp( ReturnNode );
	}
    ...
    void getPortId( unsigned x )
    {
        portNumber = x;
    }

	void doPortIdHelp( const char * a, const char * b )
	{
		printHelp( DaNode );
	}

	void doDaHelp( const char * a, const char * b )
	{
		printHelp( InputNumberNode );
	}

    void getDaValue( unsigned x )
    {
        da = x;
    }

	void doDaValueHelp( const char * a, const char * b )
	{
		printHelp( SaNode );
	}

	void doSaHelp( const char * a, const char * b )
	{
		printHelp( InputNumberNode );
	}

    void getSaValue( unsigned x )
    {
        sa = x;
    }
...
struct SimulGrammar : public grammar< SimulGrammar >
{
    ...
    struct definition
    {
        definition( SimulGrammar const& /*self*/ )
        {
            ...
            SaValueRuleNode = uint_p[ &getSaValue ] >> 
			...
			VhCloseBraceRuleNode 	= CloseBraceTok[ &acceptVhCloseBraceTok ] >> (VhCloseBraceHelp | SimulEol | VhDot1adRuleNode | VhDot1qRuleNode);
        ...
        rule< ScannerT >	HelpTok, ExitTok, EndTok, Dot1adTok, Dot1qTok, CosTok,
                            VlanIdTok, OpenBraceTok, CloseBraceTok,
                            expression, 
                            SimulHelp, 
                            ExitRuleNode, ExitHelp, ExitEol,
                            EndRuleNode, EndHelp, EndEol,
                            PortTok, DaTok, SaTok,
                            PortRuleNode, PortHelp, PortIdRuleNode, PortIdHelp,
                            DaRuleNode, DaHelp, 
                            SaRuleNode, SaHelp,
                            DaValueRuleNode, DaValueHelp,
                            SaValueRuleNode, SaValueHelp,
                            SimulEol,
                            VhRuleNode, VlanHeaderHelp,
                            VhDot1adRuleNode, VhDot1qRuleNode, Vh1Help,
                            VhCosRuleNode, VhCosHelp, VhCosValueNode, VhCosValueHelp,
                            VhOpenBraceRuleNode, VhOpenBraceHelp,
