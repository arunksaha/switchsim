...
DataPlaneMgr::DataPlaneMgr() { MyDataPlaneMgr_ = this; }

DataPlaneMgr::~DataPlaneMgr() { MyDataPlaneMgr_ = 0; }

void DataPlaneMgr::processFrame(PortId rxPortId, Frame &frame) {
  ...
  if (rxEpu) {
    ...
    cout << "\t -> Ingress Frame at " << EpuNode.tokenString() << " "
         << rxEpu->epuId() << endl;
  } else {
    cout << "\tIngress Frame did not match any " << EpuNode.tokenString()
         << endl;
    return;
  }
  ...
  (const_cast<Epu *>(rxEpu))
      ->rewriteFrame(false, frame);  // Alert: 'false' denotes ingress
  ...
  if (fwdDomain) {
    cout << "\t -> Forwarding domain = " << VbNode.tokenString() << " "
         << fwdDomain->fdId() << endl;
  } else {
    cout << "\t" << EpuNode.tokenString()
         << " does not belong to any domain. Frame dropped." << endl;
    return;
  }
...
void DataPlaneMgr::epuTransmit(Epu const *txEpu, Frame frame) {
  PortId txPortId = txEpu->portId();
  Port * txPort   = portMgr()->port(txPortId);
  assert(txPort);

  ///////////////////////   Egress Tag operation  /////////////////
  (const_cast<Epu *>(txEpu))
      ->rewriteFrame(true, frame);  // Alert: 'true' denotes egress
  cout << "\n\t4. Egress Frame Rewrite:" << endl;
  cout << "\t -> At " << EpuNode.tokenString() << " " << txEpu->epuId() << " ("
       << PortNode.tokenString() << " = " << txPortId
       << "), after egress rewrite, Frame := " << frame << endl;
