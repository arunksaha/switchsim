...
std::ostream& operator<<(std::ostream& o, EpuMatchTag const& x) {
  x.dump(o);
  return o;
}

std::ostream& operator<<(std::ostream& o, EpuMatchCriteria const& x) {
  x.dump(o);
  return o;
}

std::ostream& operator<<(std::ostream& o, EpuRewriteOp const& x) {
  x.dump(o);
  return o;
}

std::ostream& operator<<(std::ostream& o, Epu const& x) {
  x.dump(o);
  return o;
}
...
void Epu::showRun(std::ostream& os) const {
...
std::ostream& Epu::dump(std::ostream& o) const {
...
PortId Epu::portId() const { return portId_; }
...
FwdDomainId Epu::fdId() const { return fdId_; }

void Epu::fdId(FwdDomainId const& fId) {
  FwdDomain* f = 0;
...
EpuId Epu::epuId() const { return epuId_; }

void Epu::epuId(EpuId const& rhs) { epuId_ = rhs; }

EpuMatchCriteria& Epu::ingressMatch() { return ingressMatch_; }

EpuMatchCriteria const& Epu::ingressMatch() const { return ingressMatch_; }

void Epu::ingressMatch(EpuMatchCriteria const& rhs) { ingressMatch_ = rhs; }

EpuRewriteOp& Epu::ingressRewrite() { return ingressRewrite_; }

EpuRewriteOp const& Epu::ingressRewrite() const { return ingressRewrite_; }

void Epu::ingressRewrite(EpuRewriteOp const& rhs) {
  assert(rhs.valid());

  ingressRewrite_ = rhs;
}

EpuRewriteOp& Epu::egressRewrite() { return egressRewrite_; }

EpuRewriteOp const& Epu::egressRewrite() const { return egressRewrite_; }

void Epu::egressRewrite(EpuRewriteOp const& rhs) {
  assert(rhs.valid());

  egressRewrite_ = rhs;
}

bool Epu::adminState() const { return adminState_; }

void Epu::adminState(bool rhs) { adminState_ = rhs; }

Counter Epu::inFrames() const { return inFrames_; }

void Epu::incrInFrames() const {
  ++inFrames_;
...
Counter Epu::outFrames() const { return outFrames_; }

void Epu::incrOutFrames() const {
  ++outFrames_;
...
void Epu::rewriteFrame(bool const dir, Frame& frame) {
...
FwdDomain* Epu::myFd() const { return (fdMgr()->findFwdDomain(fdId_)); }
////////////////////////////////////////////////////

EpuMatchCriteria::EpuMatchCriteria() { clear(); }

void EpuMatchCriteria::clear() {
  isValid_    = false;
  isUntagged_ = false;
  tag_[0].clear();
  tag_[1].clear();
...
std::ostream& EpuMatchCriteria::dump(std::ostream& o) const {
  std::string untagStr = "";
...
void EpuMatchCriteria::print() const { std::cout << (*this); }

signed EpuMatchCriteria::numMatchTags() const {
  signed x = -1;
...
bool EpuMatchCriteria::isValid() const { return isValid_; }

bool EpuMatchCriteria::isUntagged() const { return isUntagged_; }

bool EpuMatchCriteria::contains(VlanHeader const& vh, signed& depth) const {
  ...
  if (myNumMatchTags == 2) {
    ...
    if (tag_[0].contains(vh.tag_[0]) && tag_[1].contains(vh.tag_[1])) {
      depth = 2;
      return true;
    }
  }
...
EpuMatchTag::EpuMatchTag() { clear(); }

void EpuMatchTag::clear() {
...
std::ostream& EpuMatchTag::dump(std::ostream& o) const {
...
bool EpuMatchTag::contains(VlanTag const& vt) const {
  bool contains = true;
...
EpuRewriteOp::EpuRewriteOp() { clear(); }

void EpuRewriteOp::clear() {
  rewriteType_ = RewriteTypeUnknown;
  numPopTags_  = DefaultNumPopTags;
  vlanHeader_.clear();
}

std::ostream& EpuRewriteOp::dump(std::ostream& o) const {
  switch (rewriteType_) {
    case RewriteTypeUnknown:
      break;
...
bool EpuRewriteOp::valid() const {
  switch (rewriteType_) {
    ...
    case RewriteTypePop:
      ...
      if ((numPopTags_ == 1) || (numPopTags_ == 2)) return true;
    ...
    case RewriteTypePush:
      ...
      if (numPopTags_ == DefaultNumPopTags) return true;
