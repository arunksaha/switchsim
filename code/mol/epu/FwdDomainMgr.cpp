...
MacTable *FwdDomainMgr::macTable() const {
  assert(macTable_);
  return macTable_;
}

void FwdDomainMgr::showMacTable() const { macTable()->showMacEntries(); }

FwdDomain *FwdDomainMgr::createFwdDomain(FwdDomainId const &fwdDomainId) {
...
void FwdDomainMgr::deleteFwdDomain(FwdDomainId const &fwdDomainId) {
...
FwdDomain *FwdDomainMgr::findFwdDomain(FwdDomainId const &fwdDomainId) {
...
void FwdDomainMgr::showFds(FwdDomainId const &fdId,
                           bool const &       counterFlag) const {
  ...
  if (fdId == 0) {
    ...
    for (it = fwdDomainSet_.begin(); it != fwdDomainSet_.end(); it++) {
      eid = (*it).first;
      e   = (*it).second;
...
void FwdDomainMgr::dumpHeader(bool const &counterFlag) const {
  cout << "--------------------------------------------------------------------"
          "-----"
       << endl;
  cout << VbNode.tokenString() << " Id \t "
       << "Type \t\t " << EpuNode.tokenString() << " Id(s)";
  if (counterFlag) {
