...
Port* PortMgr::port(const PortId& pId) const {
  assert(pId < numPorts_);

  return (port_[pId]);
}

void PortMgr::showRun(ostream& os) const {
  for (unsigned ui = 0; ui < numPorts_; ++ui) {
    assert(port_[ui]);
...
void PortMgr::showPorts(PortId const& pId, bool const& counterFlag) const {
  if (pId >= numPorts_) {
...
unsigned PortMgr::numPorts() const { return numPorts_; }

void PortMgr::dumpHeader(bool const& counterFlag) const {
  cout << "--------------------------------------------------------------------"
          "-----"
       << endl;
  cout << PortNode.tokenString() << " Id \t " << EpuNode.tokenString()
       << " member Id(s)";
  if (counterFlag) {
