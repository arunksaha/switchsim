...
void Port::addEpu(Epu* e) {
  assert(e);
  epuList_.push_back(e);
}

void Port::removeEpu(Epu* e) {
  assert(e);
...
void Port::showRun(ostream& os) const {
  EpuListConstItr it;
...
Counter Port::inFrames() const { return inFrames_; }

void Port::incrInFrames() { ++inFrames_; }

Counter Port::outFrames() const { return outFrames_; }

void Port::incrOutFrames() { ++outFrames_; }

Epu const* Port::ingressMatchClassify(Frame const& frame) {
  ...
  for (it = epuList_.begin(); it != epuList_.end(); ++it) {
    ...
    // cout << "\t\t\tTrying " << candidateEpu->epuId() << ": " <<
...
void Port::dump(bool const& counterFlag) const {
  if (epuList_.empty()) {
    return;
  }
