...
void acceptPortHelpEol(const char* a, const char* b) { HelpFlag = true; }
...
void doPortNoHelp(const char* a, const char* b) { printHelp(EpuNode); }

void rememberNo(const char* a, const char* b) { noFlag = true; }

void doPortEpuHelp(const char* a, const char* b) { printEpuIdHelp(); }

void getPortEpuId(const unsigned int x) { PortParam1 = x; }

void doPortEpuIdHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
void acceptPortEpuIdEol(const char* a, const char* b) {
  if (noFlag) {
    ...
    if (success) {
      ...
      cout << "Command REJECTED: Probably this EPU exists in some other Port."
           << endl;
    }
  }
...
void doPortExitHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
void doPortEndHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
struct PortGrammar : public grammar<PortGrammar> {
  ...
  struct definition {
    definition(PortGrammar const& /*self*/) {
      ...
      // Production rules
      expression =
          PortHelp | NoRuleNode | EpuRuleNode | ExitRuleNode | EndRuleNode;
    ...
    rule<ScannerT> HelpTok, EpuTok, expression, PortHelp, EpuRuleNode, EpuHelp,
        NoTok, NoRuleNode, NoHelp, EpuIdRuleNode, EpuIdHelp, EpuIdEol, ExitTok,
        ExitRuleNode, ExitHelp, ExitEol, EndTok, EndRuleNode, EndHelp, EndEol;

    rule<ScannerT> const& start() const { return expression; }
  };
};

void PortParser::portParser(string const& inputString, bool& helpRequested,
                            ConfigState* c) {
  PortParam1       = 0;
  ParserModeChange = 0;
  HelpFlag         = false;
