...
GlobalParser::GlobalParser() : gph_(0) {
...
void doSaveHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
void doMacTableVbIdHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
void doGlobalConfigHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
void acceptGlobalHelpEol(const char* a, const char* b) { HelpFlag = true; }

void acceptGlobalRunEol(const char* a, const char* b) { gph_Copy_->showRun(); }

void acceptSaveEol(const char* a, const char* b) { gph_Copy_->saveConfig(); }

void doHostnameHelp(const char* a, const char* b) {
  printf("  %12s        %s\n", "character string",
         "name of this device (max 16 alphanumeric characters)");
}
...
void getGlobalPortId(const unsigned int x) { GlobalParam1 = x; }

void getGlobalVbId(const unsigned int x) { GlobalParam1 = x; }

void getGlobalEpuId(const unsigned int x) { GlobalParam1 = x; }

void acceptCountersFlag(const char* a, const char* b) { CountersFlag = true; }
...
void doSimulHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
struct GlobalGrammar : public grammar<GlobalGrammar> {
  ...
  struct definition {
    definition(GlobalGrammar const& /*self*/) {
      ...
      ShowRuleNode =
          ShowTok >> (ShowHelp | ShowRunRuleNode | ShowPortRuleNode |
      ...
      ShowPortRuleNode = PortTok >> (PortHelp | PortIdRuleNode |
                                     PortIdCountersNode | PortIdEol);
      PortHelp       = HelpTok[&doGlobalShowPortHelp];
      PortIdRuleNode = uint_p[&getGlobalPortId] >>
                       (PortIdHelp | PortIdCountersNode | PortIdEol);
      PortIdHelp = HelpTok[&doGlobalPortIdHelp];
      PortIdCountersNode =
          CountersTok[&acceptCountersFlag] >> (PortIdCountersHelp | PortIdEol);
      PortIdCountersHelp = HelpTok[&doGlobalPortIdCountersHelp];
      PortIdEol          = end_p[&acceptGlobalPortIdEol];

      ShowVbRuleNode =
          VbTok >> (VbHelp | VbIdRuleNode | VbIdCountersNode | VbIdEol);
      VbHelp = HelpTok[&doGlobalShowVbHelp];
      VbIdRuleNode =
          uint_p[&getGlobalVbId] >> (VbIdHelp | VbIdCountersNode | VbIdEol);
      VbIdHelp = HelpTok[&doGlobalVbIdHelp];
      VbIdCountersNode =
          CountersTok[&acceptCountersFlag] >> (VbIdCountersHelp | VbIdEol);
      VbIdCountersHelp = HelpTok[&doGlobalVbIdCountersHelp];
      VbIdEol          = end_p[&acceptGlobalVbIdEol];

      ShowEpuRuleNode =
          EpuTok >> (EpuHelp | EpuIdRuleNode | EpuIdCountersNode | EpuIdEol);
      EpuHelp = HelpTok[&doGlobalShowEpuHelp];
      EpuIdRuleNode =
          uint_p[&getGlobalEpuId] >> (EpuIdHelp | EpuIdCountersNode | EpuIdEol);
      EpuIdHelp = HelpTok[&doGlobalEpuIdHelp];
      EpuIdCountersNode =
          CountersTok[&acceptCountersFlag] >> (EpuIdCountersHelp | EpuIdEol);
      EpuIdCountersHelp = HelpTok[&doGlobalEpuIdCountersHelp];
      EpuIdEol          = end_p[&acceptGlobalEpuIdEol];

      ShowMacRuleNode =
          MacTableTok >> (MacTableHelp | MacTableVbRuleNode | MacTableEol);
      ...
      MacTableVbIdRuleNode =
          uint_p[&getGlobalVbId] >> (MacTableVbIdHelp | MacTableEol);
      ...
      HostnameStringHelp = (HelpTok)[&doHostnameStringHelp];
    ...
    rule<ScannerT> expression, GlobalHelp, ShowRuleNode, ConfigRuleNode,
        HelpTok, ShowTok, ConfigTok, PortTok, VbTok, EpuTok, CountersTok,
        MacTableTok, ShowHelp, ShowPortRuleNode, ShowVbRuleNode,
        ShowEpuRuleNode, PortHelp, PortIdRuleNode, VbHelp, VbIdRuleNode,
        EpuHelp, EpuIdRuleNode, RunTok, ShowRunRuleNode, RunHelp, RunEol,
        PortIdCountersNode, PortIdCountersHelp, VbIdCountersNode,
        VbIdCountersHelp, EpuIdCountersNode, EpuIdCountersHelp, PortIdHelp,
        PortIdEol, VbIdHelp, VbIdEol, EpuIdHelp, EpuIdEol, ShowMacRuleNode,
        MacTableHelp, MacTableEol, MacTableVbRuleNode, MacTableVbHelp,
        MacTableVbIdRuleNode, MacTableVbIdHelp, ConfigHelp, ConfigEol,
        SaveRuleNode, SaveTok, SaveHelp, SaveEol, HostnameRuleNode,
        HostnameHelp, HostnameStringRuleNode, HostnameStringHelp, HostnameEol,
...
void GlobalParser::globalParser(string const& inputString, bool& helpRequested,
                                ConfigState* c) {
  using namespace GlobalParserNameSpace;
