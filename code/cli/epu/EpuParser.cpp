...
enum Direction { DirIngress = 0, DirEgress, DirUnknown };
...
ParseNode MatchNode("match",
                    "specify match criteria: which ingress frames at the "
                    "container port are mapped to this?");
...
// ParseNode PtNode		( "passthrough", "associate this EPU to a
// passthrough (ELINE)" );
ParseNode PopNode(
    "pop",
    "specify number of VLAN tags to be popped from the top of the frame");
ParseNode PushNode("push",
                   "specify VLAN tags to be pushed at the top of the frame");
ParseNode SwapNode("swap", "specify VLAN tags to be swapped in ");
ParseNode AnyNode("any", "match any");
ParseNode UntagCommaNode(",",
                         "specify tag matching criteria followed by comma");
ParseNode PtagCommaNode(",", "specify VLAN ID(s) followed by comma");
ParseNode NumberListNode("number-list",
                         "single number, or comma separated list of number(s) "
                         "or ranges(s), e.g. 10,20-30,40");
...
void doEpuExitHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
void doEpuEndHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
/////////////////////////  vbridge start ///////////////////////
void getEpuVbId(const unsigned x) { vbId = x; }

void doEpuVbHelp(const char* a, const char* b) { printVbIdHelp(); }

void doEpuVbIdHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
//////////////////////  passthrough start //////////////////////
void getEpuPtId(const unsigned x) { vbId = x; }

void doEpuPtHelp(const char* a, const char* b) {
  printf("  %12s        %s\n", "passthrough-id",
         "passthrough identifier (globally unique)");
}

void doEpuPtIdHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
void acceptIngressDirection(const char* a, const char* b) { dir = DirIngress; }
...
void doListCosHelp(const char* a, const char* b) { printHelp(NumberListNode); }
...
void doVlanIdHelp(const char* a, const char* b) { printHelp(NumberListNode); }

void doVlanListHelp(const char* a, const char* b) { printHelp(CloseBraceNode); }
...
void doEpuPtagCommaHelp(const char* a, const char* b) { printHelp(VlanIdNode); }
...
void doEpuIngRwHelp(const char* a, const char* b) { printHelp(PopNode); }
...
void getEpuNumPopTags(const unsigned x) { epuRewriteOp.numPopTags_ = x; }
...
void doEpuEgressHelp(const char* a, const char* b) { printHelp(RewriteNode); }

void acceptEgressDirection(const char* a, const char* b) { dir = DirEgress; }
...
void acceptEpuHelpEol(const char* a, const char* b) { HelpFlag = true; }
...
void getLowerNumber(const unsigned x) { lower = x; }
...
struct EpuGrammar : public grammar<EpuGrammar> {
  ...
  struct definition {
    definition(EpuGrammar const& /*self*/) {
      ...
      // Production rules
      expression = EpuHelp | ExitRuleNode | EndRuleNode | ShutdownRuleNode |
                   VbRuleNode |
      ...
      */
      ...
      IngRwPopNumNode =
          uint_p[&getEpuNumPopTags] >> (IngRwPopNumHelp | IngRwPopNumEol);
      IngRwPopNumHelp = (HelpTok)[&doEpuIngRwPopNumHelp];
      IngRwPopNumEol  = end_p[&acceptEpuIngRwPopNumEol];

      IngressMatchRuleNode = MatchTok >> (IngMatchHelp | UntagRuleNode |
                                          Dot1adRuleNode | Dot1qRuleNode);
      ...
      OpenBraceNode = OpenBraceTok[&acceptOpenBraceTok] >>
                      (OpenBraceHelp | VlanIdRuleNode | PtagRuleNode);
      OpenBraceHelp = (HelpTok)[&doOpenBraceHelp];

      VlanIdRuleNode =
          VlanIdTok[&acceptVlanIdTok] >> (VlanIdHelp | VlanIdListNode);
      ...
      CloseBraceNode =
          CloseBraceTok[&acceptCloseBraceTok] >>
      ...
      EgressRuleNode =
          EgressTok[&acceptEgressDirection] >> (EgressHelp | EgressRwRuleNode);
      EgressHelp = (HelpTok)[&doEpuEgressHelp];
      EgressRwRuleNode =
          RewriteTok >> (EgRwHelp | EgRwPushRuleNode | EgRwSwapRuleNode);
      EgRwHelp         = (HelpTok)[&doEpuEgressRwHelp];
      EgRwPushRuleNode = PushTok[&acceptPushTok] >>
                         (VlanHeaderHelp | VhDot1adRuleNode | VhDot1qRuleNode);
      EgRwSwapRuleNode = SwapTok[&acceptSwapTok] >>
                         (VlanHeaderHelp | VhDot1adRuleNode | VhDot1qRuleNode);
      ...
      VhCloseBraceRuleNode = CloseBraceTok[&acceptVhCloseBraceTok] >>
                             (VhCloseBraceHelp | VhCloseBraceEol |
                              VhDot1adRuleNode | VhDot1qRuleNode);
      ...
      NumberListEntry =
          (uint_p[&getLowerNumber] >> ch_p('-') >> uint_p[&getHigherNumber]) |
    ...
    rule<ScannerT> HelpTok, ExitTok, IngressTok, EgressTok, VbTok, PtTok,
        MatchTok, UntagTok, PtagTok, PopTok, Dot1adTok, Dot1qTok, CosTok,
        VlanIdTok, AnyTok, CommaTok, RewriteTok, expression, IngressRuleNode,
        IngressRewriteRuleNode, IngressMatchRuleNode, MatchHelp, UntagHelp,
        UntagEol, UntagCommaNode, Dot1adHelp, OpenBraceNode, OpenBraceExp,
        Dot1qHelp, UntagCommaHelp, CosHelp, CosListNode, NumberLIst,
        CosListHelp, OpenBraceTok, OpenBraceHelp, VlanIdHelp, VlanIdListNode,
        VlanListHelp, CloseBraceExp, CloseBraceNode, CloseBraceTok,
        CloseBraceHelp, CloseBraceEol, PtagRuleNode, PtagHelp,
        PtagCommaRuleNode, PtagCommaHelp, EgSuffixTree, NumberList,
        NumberListEntry, IngMatchHelp, UntagRuleNode, Dot1adRuleNode,
        Dot1qRuleNode, CosRuleNode, VlanIdRuleNode, UntagCommaRuleNode,
        IngRwHelp, IngRwPopNode, IngRwPopHelp, IngRwPopNumNode, IngRwPopNumHelp,
        IngRwPopNumEol, EpuHelp, ExitRuleNode, ShutdownRuleNode, VbRuleNode,
        PtRuleNode, EgressRuleNode, ExitHelp, ExitEol, ShutdownTok,
        ShutdownHelp, ShutdownEol, VbHelp, VbIdNode, VbIdHelp, VbIdEol, PtHelp,
        PtIdNode, PtIdEol, PtIdHelp, IngressHelp, EgressHelp, EndTok,
        EndRuleNode, EndHelp, EndEol, VhDot1adRuleNode, VhDot1qRuleNode,
        Vh1Help, VhCosRuleNode, VhCosHelp, VhCosValueNode, VhCosValueHelp,
        VhOpenBraceRuleNode, VhOpenBraceHelp, VhVlanIdRuleNode, VhVlanIdHelp,
        VhVlanIdValueRuleNode, VhVlanIdValueHelp, VhCloseBraceRuleNode,
        VhCloseBraceHelp, VhCloseBraceEol, EgressRwRuleNode, EgRwHelp,
        EgRwPushRuleNode, EgRwSwapRuleNode, VlanHeaderHelp, PushTok, SwapTok;

    rule<ScannerT> const& start() const { return expression; }
  };
};

bool EpuParser::epuParser(string const& inputString, bool& helpRequested,
                          ConfigState* c) {
  using namespace EpuParserNameSpace;
