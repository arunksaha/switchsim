...
ParserMain::ParserMain()
  ...
  epuPromptSuffix_ =
      "(config-" + PortNode.tokenString() + "-" + EpuNode.tokenString() + ")#";
...
MolVp* ParserMain::molVp() const { return (molVp_); }

void ParserMain::molVp(MolVp* m) {
  assert(!molVp_ && m);
  molVp_ = m;
}

ConfigState* ParserMain::configState() const {
  assert(MyParserMain_);
  return (MyParserMain_->cfgState_);
}

string ParserMain::globalPrompt() const {
  return (configState()->hostname() + globalPromptSuffix_);
}

string ParserMain::configPrompt() const {
  return (configState()->hostname() + configPromptSuffix_);
}

string ParserMain::portPrompt() const {
  return (configState()->hostname() + portPromptSuffix_);
}

string ParserMain::epuPrompt() const {
  return (configState()->hostname() + epuPromptSuffix_);
}

string ParserMain::vbPrompt() const {
...
void ParserMain::printPrompt(ParserMode m) const {
  cout << endl;
...
void ParserMain::readSavedConfig() const {
  string   line;
  ifstream savedFile;
...
void ParserMain::parseSavedLine(string const& inputString) const {
  bool helpReq = false;
...
string ConfigState::hostname() const { return hostname_; }

void ConfigState::hostname(string const& s) { hostname_ = s; }

vector<string>& ConfigState::commandLines() { return commandLines_; }
