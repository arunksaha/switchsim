...
void acceptConfigHelpEol(const char* a, const char* b) { HelpFlag = true; }
...
void doConfigPortHelp(const char* a, const char* b) { printPortIdHelp(); }

void getConfigPortId(const unsigned int x) { ConfigParam1 = x; }

void doConfigPortIdHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
void doConfigExitHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
void doConfigEndHelp(const char* a, const char* b) { printHelp(ReturnNode); }
...
struct ConfigGrammar : public grammar<ConfigGrammar> {
  ...
  struct definition {
    ...
    rule<ScannerT> HelpTok, PortTok, ExitTok, expression, ConfigHelp,
        PortRuleNode, PortHelp, PortIdRuleNode, PortIdHelp, PortIdEol,
...
void ConfigParser::configParser(string const& inputString, bool& helpRequested,
                                ConfigState* c) {
  ConfigParam1     = 0;
  ParserModeChange = 0;
  HelpFlag         = false;
