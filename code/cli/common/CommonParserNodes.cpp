...
ParseNode ShutdownNode("shutdown", "shutdown current configuration object");
...
ParseNode VbNode("domain", "specify forwarding domain of the ingress frames");
...
ParseNode CloseBraceNode(")", "end 'vlanid' value list with close parenthesis");
...
void printEpuIdHelp() {
  ...
  cout << "\t" << epu << "-id\t\t"
       << "specify a " << epu << " id to create/configure/delete" << endl;
}

void printVbIdHelp() {
  std::string vb = VbNode.tokenString();
  cout << "\t" << vb << "-id\t\t"
       << "specify a " << vb << " id for forwarding behavior association"
       << endl;
}
